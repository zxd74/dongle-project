...
public class ShardedJedisUtils {
	...
	private static final int REDIS_DEFAULT_DB = PropertyGetter.getInt("redis.default.db", 0);
	private static final String REDIS_PASSWORD = PropertyGetter.getString("redis.password");
	private static final String REDIS_CLUSTER = PropertyGetter.getString("redis.cluster");
	...
	private void init(int db){
		try {
			...
			String[] array = REDIS_CLUSTER.split(Constants.SIGN_COMMA);
			...
			for (int i = 0; i < array.length; i++) {
				...
				host = StringUtils.buildString(REDIS_SERVERS_URI, REDIS_PASSWORD, StringUtils.trim(array[i]), db);
	...
	public static ShardedJedisUtils getInstance(){
		if(instance == null){
			instance = new ShardedJedisUtils(REDIS_DEFAULT_DB);
		}
		return instance;
	}
