...
-XX:+PrintGC -Xloggc:/data/logs/apps/nvwa/nvwa-openapi-dsp/gc_$1.log \
...
pid=`ps -eo pid,args | grep nvwa-openapi-dsp | grep $1 | grep java | grep -v grep | awk '{print $1}'`
...
java -Dmodule=$1 $JAVA_OPTS -cp $PHOME/conf:$PHOME/lib/* com.iwanvi.nvwa.openapi.dsp.bootstrap.HttpServer $1 > /dev/null 2>&1 &
