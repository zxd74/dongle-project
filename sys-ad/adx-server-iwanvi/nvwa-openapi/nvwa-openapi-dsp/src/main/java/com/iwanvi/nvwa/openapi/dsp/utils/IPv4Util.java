...
/**
 * http://www.blogjava.net/javagrass/archive/2011/07/08/353921.html
 * 
 * IP ——> 整数：
	把IP地址转化为字节数组
	通过左移位（<<）、与（&）、或（|）这些操作转为int
...
public class IPv4Util {
	...
	private final static int INTEGER_0 = 0;
	...
	public static int ipToInt(String ipAddr) {
		return INTEGER_0;
		//20161102 userinfo clientIp invalid
//		try {
...
//		}
	...
	public static String intToIp(int ipInt) {
		...
		try {
			ip = new StringBuilder().append(((ipInt >> 24) & 0xff)).append('.').append((ipInt >> 16) & 0xff).append('.')
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(ipAndMask + " --> IP 地址段：[ " + IPv4Util.intToIp(ipscope[0]) + ","
		...
		System.out.println(ipAddr1 + " , " + ipMask1 + "  --> IP地址段 ：[ " + IPv4Util.intToIp(ipscope1[0]) + ","
