...
JAVA_OPTS="-server -Xms256m -Xmx256m -Dmodule=$1 -Dsun.rmi.dgc.server.gcInterval=3600000 \
...
pid=`ps -eo pid,args | grep mockdsp|grep $1 | grep java | grep -v grep | awk '{print $1}'`
...
java $JAVA_OPTS -cp $PHOME/conf:$PHOME/lib/* com.iwanvi.nvwa.adx.connector.Application $1 mockdsp > /dev/null 2>&1 &