...
pid=`ps -eo pid,args | grep adserv-ngx | grep $1 | grep java | grep -v grep | awk '{print $1}'`
...
java $JAVA_OPTS -cp $PHOME/conf:$PHOME/lib/* com.iwanvi.adserv.ngx.boot.Application $1 adserv-ngx > /dev/null 2>&1 &