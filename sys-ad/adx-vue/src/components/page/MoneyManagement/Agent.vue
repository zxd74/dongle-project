...
<script>
    ...
    import {sumHistory} from "@/api/Api.js";
    import {agentRechargesumByDate} from "@/api/Api.js";
    import {rechargegetList} from "@/api/Api.js";
    import {agentRechargeAdd} from "@/api/Api.js";
    import {agentRecharge,reportForCast,agentrechargegetList} from "@/api/Api.js";
    ...
    export default {
        ...
        methods: {
            ...
            recharge() {
                ...
                // 代理商充值/退款
                let params = {agentId: this.id, type: 1, rechargePrice: this.prices}
                agentRechargeAdd(params).then(res => {
                    let data = res.data;
                    if (data.code != 'A000000') {
                        this.$message.error(data.message);
                    } else {
            ...
            EditRefund() {
                let id = this.$route.query.id
                // 代理商充值/退款
                let params = {agentId: this.id, type: 2, rechargePrice: this.refundPrice}
                agentRechargeAdd(params).then(res => {
                    let data = res.data;
                    if (data.code != 'A000000') {
                        this.$message.error(data.message);
            ...
            tendency() {
                ...
                if (this.record == 2) {
                      // 充值echaers
                            ...
                            agentRechargesumByDate(par).then(res => {
                                let data = res.data;
                                this.createDay = data.data.date
                            ...
                            agentrechargegetList(table).then(res => {
                                    console.log(res)
                                let data = res.data;
