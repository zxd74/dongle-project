<template>
    <div class="box">
        ...
        <div class="border">创意列表:</div>
        <el-table :data="IdearData" border style="width: 100%" ref="multipleTable" id="table2">
                <el-table-column prop="entId" label="创意ID">
                </el-table-column>
                <el-table-column prop="entityName" label="创意名称">
                </el-table-column>
                ...
                <!-- <el-table-column prop="entityActive" label="激活数">
                </el-table-column>
                ...
                <el-table-column align="center" width="110">
                    <template slot-scope="scope" align="center">
                        <el-button size="small" type="success" @click="IdearReport(scope.$index, scope.row)">创意报告</el-button>
                    </template>
                </el-table-column>
            </el-table>
        <div class="border">投放分日数据:</div>
        <el-table :summary-method="getSummaries" show-summary :data="DetailTable" border style="width: 100%" ref="multipleTable" id="table3">
...
import { quotaPutlistByDay,quotaEntitylistByPid} from "@/api/Api.js";
// 导出模块
...
const cityOptions = [ '展现量', '点击量','展现率%', '点击率', '收入', 'CPM','CPC'];
export default {
  name: "basecharts",
  components: {
    // Schart
  ...
  mounted() {
   ...
   methods: {
      ...
      search(){
        ...
        var echartsDate  = [];
          let params = {
            putId:this.$route.query.id,
            sTime:this.startDay,
            eTime:this.endDay,
            putType:19
            }
          quotaPutlistByDay(params).then(res=>{
            ...
            this.DetailTable = res.data.result 
            // console.log(res)
            ...
            //    创意列表
                ...
                quotaEntitylistByPid(param).then(res=>{
                   if(res.data.code != 'A000000') {
                        this.$message.error(data.code);
                        }
                        console.log(res)
                    this.IdearData = res.data.result.data
                });
    ...
    IdearReport(index,row){
        this.$router.push({path:'/datareport/ideartwo', query:{entId:row.entId}})
      },
            //   导出table方法
    exportExcel () {
            ...
            FileSaver.saveAs(new Blob([wbout], { type: 'application/octet-stream' }), '创意列表.xlsx')

            FileSaver.saveAs(new Blob([wbout1], { type: 'application/octet-stream' }), '投放分日数据.xlsx')
