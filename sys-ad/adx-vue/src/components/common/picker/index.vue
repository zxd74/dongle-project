...
<div id="app">
    <div class="flex">
        ...
        <div>
            <calendar :range="calendar2.range" :lunar="calendar2.lunar" :value="calendar2.value" :begin="calendar2.begin" :end="calendar2.end" @select="calendar2.select" :now="now" :data="data"></calendar>
        </div>
    </div>
...
export default {
    ...
    data(){
        return {
            ...
            calendar2:{
                ...
                lunar:false, //显示农历
                begin:[2018,10,25], //可选开始日期
                end:[2020,2,16], //可选结束日期
                select(begin,end){
    ...
    watch: {
        'time.add_time': function(val,oldval) {
            let beginY = this.time.add_time[0].getFullYear();
            let beginM = this.time.add_time[0].getMonth();
            let beginD = this.time.add_time[0].getDay();
            this.calendar2.begin = [beginY,beginM,beginD];
            this.now = this.time.add_time[0];
            console.log(this.calendar2.begin);
        }
    //     time:{
    // 　　 immediate:true,
